...
public class Container {
  ...
  public void createRotateBitmaps(int[] draws, int rotation, GameView gameView) {
    Matrix m = new Matrix();
    m.postRotate(rotation);

    Bitmap bitmap;
    for (int res : draws) {
      bitmap = BitmapFactory.decodeResource(gameView.getResources(), res);
      bitmaps.add(Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), m, false));
    }
  }

  public void createBitmaps(int[] draws, GameView gameView) {
    Bitmap bitmap;
    for (int res : draws) {
      bitmap = BitmapFactory.decodeResource(gameView.getResources(), res);
